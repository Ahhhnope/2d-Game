...
import GameOBJ.ModelGameBossOne;
import GameOBJ.TheEndGame;
import GameOBJ.WinAndEndBoss;
import GameOBJ.player;
import GameOBJ.rocket;
import GameOBJ.rocketBossOne;
...
public class GameLeverOne extends JComponent {
    ...
    private ModelGameBossOne boss;

    private List<rocketBossOne> bossBullets;
    ...
    public GameLeverOne() {
        backgroundImage = new ImageIcon(getClass().getResource("/GameImage/GameLVone.jpeg")).getImage();
...
// Thêm phương thức returnToMainMenu() trong class PlaneGame
    ...
    private void drawGame() {
        ...
        if (showCountdown) {
            ...
            // Giảm kích thước ảnh
            Image introImage = new ImageIcon(getClass().getResource("/GameImage/thongtinboss.png")).getImage();
        ...
        if (player.isALive()) {
            ...
            synchronized (bossBullets) {
                for (int i = 0; i < bossBullets.size(); i++) {
                    rocketBossOne bullet = bossBullets.get(i);
                    if (bullet != null) {
                        bullet.draw(g2);
                    }
                }
    ...
    private void showEndGameScreen() {
        ...
        WinAndEndBoss endScreen = new WinAndEndBoss();
...
// Sửa phương thức resetGame
    ...
    private void bossShooting() {
        if (boss.isActive() && player.isALive() && !showCountdown && !showIntroScreen) {
            // Approximately every 2 seconds (if game runs at 60 FPS)
            if (Math.random() < 0.01) {  // Adjust probability to control firing rate
                // Get boss position (center of boss)
                ...
                // Create a new boss bullet targeting the player
                rocketBossOne bullet = new rocketBossOne(bossX, bossY, playerX, playerY);
    ...
    private void showWinScreen() {
        ...
        WinAndEndBoss winScreen = new WinAndEndBoss();
